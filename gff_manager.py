...
Functions, -f:
  ...
  add_len	Add length (-int) to the start and end point of each feature in
		a GFF file
		  output: [gff_file].[int]_add_len
  ...
  overlap+	Describe the feature, length, start and end points, and
		...
		and secondary (-gff2) gff file.
		  req: -gff2
		  output: print to screen
  orientation	Determine if any GFF features are not in the positive (start <
		end) oreintation.
		  output: print to screen
  rm_iden_crd	Remove duplicates of features with identical start-stop coords
...
def function_addLength(gff,addLn):
	inp = open(gff)
	out = open("%s.%s_add_len"%(gff,addLn),"w")
	for line in inp:
		lineLst = line.strip().split("\t")
		out_l = lineLst[:]
		out_l[3] = str(int(lineLst[3])-int(addLn))
		out_l[4] = str(int(lineLst[4])+int(addLn))
		out.write("\t".join(out_l)+"\n")
	out.close()
	inp.close()

def function_orientation(gff):
	inp = open(gff)
	for line in inp:
		if not line.startswith("#"):
			lineLst = line.strip().split("\t")
...
def function_rm_iden_crd(gff):
	...
	for line in inp:
		if not line.startswith("#"):
			...
			id_tup = (seq,type,start,end)
			if id_tup not in d:
				d[id_tup] = [line]
			else:
				d[id_tup].append(line)
	inp.close()
	
	import random
	out = open(gff+".rm_coord_dup","w")
	for id_tup in d:
		lines_l = d[id_tup]
		if len(lines_l) == 1:
			out.write(lines_l[0])
		else:
			random_line = random.choice(lines_l)
			out.write(random_line)
	out.close()
...
def main():
	...
	if func == None or infile == None:
		...
		function_addLength(infile,inp_int)
	elif func == "orientation":
		function_orientation(infile)
	elif func == "rm_iden_crd":
